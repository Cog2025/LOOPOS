...
import { OS, PlantMaintenancePlan } from '../../types';
...
// Helper para imagens com dimensões
interface ImageData {
    base64: string;
    width: number;
    height: number;
...
// --- 1. RELATÓRIO DO PLANO DE MANUTENÇÃO (COMPLETO & CORRIGIDO) ---
export const generateMaintenancePlanReport = (tasks: PlantMaintenancePlan[], plantName: string): jsPDF => {
    // Orientação Paisagem para caber todas as colunas
    const doc = new jsPDF({ orientation: 'landscape' });

    // Cabeçalho
    doc.setFontSize(18);
    doc.text(`Plano de Manutenção: ${plantName}`, 14, 15);
    
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Total de Tarefas: ${tasks.length}`, 14, 22);
    doc.text(`Gerado em: ${new Date().toLocaleString()}`, 200, 22);

    // Preparar dados da tabela
    const tableBody = tasks.map(t => [
        t.asset_category || 'Geral',      
        t.title,                          
        getFrequencyLabel(t.frequency_days), // ✅ CORREÇÃO: frequency_days (conforme types.ts)
        (t as any).role || 'Técnico',        // ✅ CORREÇÃO: cast as any para evitar erro de TS + fallback
        t.criticality || 'Média',         
        t.classification1 || '-',         
        t.classification2 || '-',         
        formatMinutes(t.estimated_duration_minutes), 
        t.subtasks?.length || 0           
    ]);

    // Ordenar por Ativo -> Frequência
    tableBody.sort((a, b) => String(a[0]).localeCompare(String(b[0])));

    autoTable(doc, {
        startY: 28,
        head: [['Ativo', 'Descrição da Tarefa', 'Frequência', 'Resp.', 'Criticidade', 'Class. 1', 'Class. 2', 'Tempo', 'Checklist']],
        body: tableBody,
        theme: 'striped',
        headStyles: { 
        ...
        columnStyles: {
            ...
            1: { cellWidth: 'auto' },                // Tarefa (expandida)
            2: { cellWidth: 20, halign: 'center' },  // Freq
            3: { cellWidth: 20, halign: 'center' },  // Resp
            4: { cellWidth: 20, halign: 'center' },  // Criticidade
            5: { cellWidth: 22, halign: 'center' },  // Class 1
            6: { cellWidth: 22, halign: 'center' },  // Class 2
            7: { cellWidth: 15, halign: 'center' },  // Tempo
            8: { cellWidth: 18, halign: 'center' }   // Checklist
        },
        // Estilização condicional (Destaque para Criticidade)
        didParseCell: (data) => {
            if (data.section === 'body' && data.column.index === 4) { 
                const val = String(data.cell.raw);
                if (val === 'Alta' || val === 'Urgente' || val === 'Alto' || val === 'Muito alto') {
...
// --- 2. RELATÓRIO DE ORDEM DE SERVIÇO (MANTIDO) ---
export const generateOSReport = async (
  ...
  for (let i = 0; i < osList.length; i++) {
    ...
    // Se for apenas 1 OS, fazemos o relatório detalhado com fotos
    if (osList.length === 1) {
        // Título da OS
